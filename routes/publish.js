...
router.post("/twitter", async function(req, res, next) {
    if (!req.body.text) {
        return res.status(400).send({
            message: "Errore durante la pubblicazione su Twitter. (È necessario fornire un testo)",
            status: "danger",
        })
    }
        ...
        try {
            ...
            if (error.errors[0].message) {
                return res.status(500).send({
                    message: "Errore durante la pubblicazione su Twitter. (" + error.errors[0].message + ")",
                    status: "danger",
                })
            } else {
                return res.status(500).send({
    ...
    try {
        ...
        return res.status(200).send({
            message: "Pubblicato su Twitter.",
            status: "primary",
            createdTweetedId: req.session.lastTweetId ? req.session.lastTweetId : "",
        })
    } catch (error) {
        console.log(error)
        try {
            ...
            axios.post("https://api.telegram.org/bot" + req.session.telegramToken + "/" + apiEndpoint + "?chat_id=" + req.session.telegramChatID, formData)
                .then(response => {
                    if (response.status === 200 && response.data.ok) {
                        res.status(200).send({
                            message: "Pubblicato su Telegram.",
                            status: "primary",
                        })
                    } else {
...
router.post("/facebook", async function(req, res, next) {
    if (!req.body.text) {
        return res.status(400).send({
            message: "Errore durante la pubblicazione su Facebook. (È necessario fornire un testo)",
            status: "danger",
        })
    }
        ...
        axios.post("https://graph.facebook.com/" + req.session.facebookPageID + "/feed?message=" + encodeURIComponent(text) + "&access_token=" + req.session.facebookToken)
            .then(response => {
                if (response.status === 200) {
                    ...
                    console.log(response.data)
                    res.status(500).send({
                        message: "Errore durante la pubblicazione su Facebook. (" + response.data + ")",
                        status: "danger",
                    })
                }
            })
            .catch(error => {
                ...
                res.status(500).send({
                    message: "Errore durante la pubblicazione su Facebook. (" + error + ")",
                    status: "danger",
                })
            })
    } catch (error) {
...
router.post("/blog", async function(req, res, next) {
    ...
    await axios.post("https://dangelodario.it/wp-json/liveblog/v1/" + req.body.post_id + "/crud/?gsm=true", {
        ...
        .then(response => {
            ...
            if (response.status === 200) {
                res.status(200).send({
                    message: "Pubblicato su Wordpress.",
                    status: "primary",
                })
            } else {
                console.log(response.data)
...
router.post("/bsky", async function(req, res, next) {
    if (!req.body.text) {
        return res.status(400).send({
            message: "Errore durante la pubblicazione su BSKY. (È necessario fornire un testo)",
            status: "danger",
        })
    }
...
router.post("/upload", upload.array("files", 10), (req, res) => {
    try {
        ...
        if (!files || files.length === 0) {
            return res.status(400).send({
                message: "Errore durante l'upload dei files multimediali.",
                status: "danger",
            })
        }
